  ...
  return (
    <>
          ...
          <div className="flex items-center gap-2 flex-1 justify-end">
            ...
            <Dialog open={isShopOpen} onOpenChange={setIsShopOpen}>
              <DialogTrigger asChild>
                <button className="flex items-center gap-1.5 px-3 py-2 rounded-full bg-card border hover:bg-accent">
              ...
              <DialogContent className="rounded-[40px] max-w-md">
                <DialogHeader><DialogTitle className="text-2xl font-bold">Portal Shop</DialogTitle></DialogHeader>
                <div className="grid gap-3 py-4">
                  ...
                  <Button onClick={() => { setIsShopOpen(false); router.push("/premium"); }} className="w-full h-14 bg-amber-500 text-white font-bold rounded-2xl">
                    <Crown className="h-5 w-5 mr-2" /> {userData?.isPremium ? "Manage Premium" : "Become Premium"}
                  </Button>
                </div>
              </DialogContent>
            </Dialog>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56 rounded-2xl">
                <DropdownMenuItem onClick={() => router.push("/settings")} className="p-3 cursor-pointer text-[10px] font-bold"><Settings className="h-4 w-4 mr-2" /> Settings</DropdownMenuItem>
                <DropdownMenuItem onClick={() => auth.signOut()} className="p-3 cursor-pointer text-destructive text-[10px] font-bold"><LogOut className="h-4 w-4 mr-2" /> Logout</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </div>
      ...
      <nav className="sm:hidden fixed bottom-0 left-0 right-0 h-16 bg-background/95 border-t z-50 flex items-center justify-around">
        <Link href="/home" className="flex flex-col items-center"><Home className="h-5 w-5" /><span className="text-[8px] font-bold">Home</span></Link>
        <button onClick={() => setIsShopOpen(true)} className="flex flex-col items-center"><ShoppingBag className="h-5 w-5" /><span className="text-[8px] font-bold">Shop</span></button>
        <Link href="/friends" className="flex flex-col items-center"><User className="h-5 w-5" /><span className="text-[8px] font-bold">Social</span></Link>
        <Link href={`/profile/${userData?.sequentialId}`} className="flex flex-col items-center"><User className="h-5 w-5" /><span className="text-[8px] font-bold">Me</span></Link>
      </nav>
    </>
  )
}
